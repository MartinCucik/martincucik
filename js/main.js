 ...
 */
{
	...
	class Pieces {
		constructor(el, options) {
			...
			Pieces.onclick=function() {console.log("AHAHO");}
			this.options = {
		...
		layout() {
			...
			for (let r = 0; r < this.options.pieces.rows; r++) {
				for (let c = 0; c < this.options.pieces.columns; c++) {
					...
					if(piece.dataset.row==2) {console.log(piece);}
		...
		document.body.append(piece);
            if (row==2 && column==0){
				piece.style.backgroundImage = `url(${this.imgsrc})`;  //do whatever you want here
				piece.onclick=function() {console.log("AHAHO");}	

			piece.className = 'piecehtml';
			
			piece.classList.add(".changed");
			piece.style.width = `${w}px`;
			piece.style.height = `${h}px`;
			piece.style.content = 'oekihg';
